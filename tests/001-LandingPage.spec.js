...
test("Landing page", async({page})=>{
    ...
    //Verify that company logo is visible.
    await expect(page.locator("//img[@alt='Carlyle Financial Logo']")).toBeVisible();
    ...
    //Currently we are hiding the Careers page, so please add carrers in the below list
    ...
    await expect(page.locator("//p[contains(text(),'Lending services provided by Carlyle Financial Inc')]")).toBeVisible();
    await expect(page.locator("(//span[@class='accessibility-btn__text'])[1]")).toBeVisible();
    await expect(page.locator("//img[@alt='Equal Housing Oportunity Logo']")).toBeVisible();
    await expect(page.locator("//iframe[@id='launcher']")).toBeVisible();
